...
(function () {
  ...
  /* ---------------------------------------------------------
     8 · Countdown  ·  10 Sep 2026, 15:00 IST
     --------------------------------------------------------- */
  (function countdown() {
    var target = new Date('2026-09-10T15:00:00+05:30').getTime();
  ...
  /* ---------------------------------------------------------
     11 · Hero → next section  ·  one-time automatic transition
     Auto-plays on its own once the hero is revealed (after a short
     pause so the artwork is actually seen), and cancels immediately
     — permanently, for the rest of the session — the moment the
     visitor touches, scrolls, or presses a key themselves.
     --------------------------------------------------------- */
  (function heroAutoTransition() {
    if (!heroEl) return;

    /* Next major section right after the hero in document order */
    var nextSection = heroEl.nextElementSibling;
    while (nextSection && nextSection.tagName && nextSection.tagName.toLowerCase() !== 'section') {
      nextSection = nextSection.nextElementSibling;
    }
    if (!nextSection) nextSection = document.getElementById('details');
    if (!nextSection) return;

    var reduceMotion = window.matchMedia &&
      window.matchMedia('(prefers-reduced-motion: reduce)').matches;
    if (reduceMotion) return; /* honour the setting: plain native scrolling only */

    var AUTO_DELAY_MS = 1400; /* pause on the hero before auto-advancing */
    var DURATION_MS = 1000;   /* within the requested 800–1200ms window */

    var triggered = false;      /* the transition has started — never repeats */
    var userInteracted = false; /* visitor took over — auto-scroll is cancelled */
    var animating = false;
    var rafId = 0;
    var autoTimer = 0;
    var restoreScrollBehavior = null; /* set while our animation owns scrolling */
    ...
    /* The moment the visitor takes control — touch, wheel, drag, or a
       navigation key — the automatic transition is cancelled for good.
       Normal scrolling continues untouched from then on.
       IMPORTANT: only listen for this once the hero is actually
       revealed — otherwise tapping "Open Invitation" (or the skip/
       audio buttons) while the video is still playing would itself
       count as "the visitor took over" and cancel the auto-scroll
       before the hero was ever shown. */
    var interruptListenersBound = false;
    ...
    function bindInterruptListeners() {
      if (interruptListenersBound) return;
      interruptListenersBound = true;
      ...
      window.addEventListener('keydown', function (e) {
        ...
        if (keys.indexOf(e.key) !== -1) markUserInteracted();
      });
    }

    function runTransition() {
      if (triggered || userInteracted) return;
      triggered = true;
      animating = true;

      /* The stylesheet sets `html { scroll-behavior: smooth }` globally.
         That makes window.scrollTo() trigger the browser's OWN smooth
         scroll on every call, fighting our custom easing frame by frame.
         Same workaround as jumpToTop(): force "auto" for the duration
         of our animation, then hand the CSS setting back afterwards. */
      var root = document.documentElement;
      ...
      var targetY = startY + nextSection.getBoundingClientRect().top;
      var delta = targetY - startY;
      var startTime = null;

      if (!delta) { cancelAnimation(); return; }

      function step(now) {
        if (!animating || userInteracted) return; /* interrupted by the user */
    ...
    function arm() {
      ...
      autoTimer = window.setTimeout(function () {
        if (!triggered && !userInteracted) runTransition();
      }, AUTO_DELAY_MS);
    }

    /* Never engage while the opening video / cover is still active —
       only once the hero is actually revealed. */
    if (body.classList.contains('is-revealed')) {
      arm();
      ...
      /* Fallback: poll briefly, then give up gracefully to native scrolling */
      var tries = 0;
      var poll = window.setInterval(function () {
        ...
        if (body.classList.contains('is-revealed')) {
          ...
          window.clearInterval(poll); /* graceful fallback: normal scrolling only */
        }
      }, 50);
    }
  })();
